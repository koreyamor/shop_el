    ...
    >
      ...
      <el-table-column align="center" label="操作" width="150">
        <template slot-scope="scope">
          ...
          <el-button type="text" size="mini" @click="ship(scope.row)" v-if="scope.row.ship_status ==='pending' && scope.row.closed === 0 && scope.row.payment_method && scope.row.refund_status === 'pending' && scope.row">
            >订单发货</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    searchEvent(e = false) {
      ...
      // return console.log(this.form.time);
      // 简单搜索
      if (typeof e === 'string') {
        //先赋值
        this.form.no = e
